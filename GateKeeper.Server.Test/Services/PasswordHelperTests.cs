...
namespace GateKeeper.Server.Test.Services
{
    ...
    public class PasswordHelperTests
    {
        ...
        public void HashPassword_InvalidSalt_ThrowsFormatException()
        {
            ...
            PasswordHelper.HashPassword("password", "this is not valid base64 salt");
        }
        #endregion
        ...
        private void SetupPasswordPolicy(int minLength, bool reqUpper, bool reqLower, bool reqDigit, bool reqSpecial, string specialChars)
        {
            ...
            _mockPasswordStrengthSection = new Mock<IConfigurationSection>();

            _mockConfiguration.Setup(c => c.GetSection("PasswordStrength")).Returns(_mockPasswordStrengthSection.Object);

            _mockPasswordStrengthSection.Setup(s => s["MinLength"]).Returns(minLength.ToString());
            _mockPasswordStrengthSection.Setup(s => s["RequireUppercase"]).Returns(reqUpper.ToString());
            _mockPasswordStrengthSection.Setup(s => s["RequireLowercase"]).Returns(reqLower.ToString());
            _mockPasswordStrengthSection.Setup(s => s["RequireDigit"]).Returns(reqDigit.ToString());
            _mockPasswordStrengthSection.Setup(s => s["RequireSpecialChar"]).Returns(reqSpecial.ToString());
            _mockPasswordStrengthSection.Setup(s => s["SpecialChars"]).Returns(specialChars);
        }
