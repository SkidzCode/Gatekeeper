...
namespace GateKeeper.Server.Test.Services
{
    ...
    public class SessionServiceTests
    {
        ...
        public void TestInitialize()
        {
            ...
            // Mock IConfiguration GetSection for DatabaseConfig (even if not strictly used in methods under test, constructor reads it)
            var mockDbConfigSection = new Mock<IConfigurationSection>();
            var mockDatabaseConfig = new Mock<DatabaseConfig>(); // Assuming DatabaseConfig is a simple POCO
            mockDbConfigSection.Setup(s => s.Get<DatabaseConfig>()).Returns(mockDatabaseConfig.Object);
            _mockConfiguration.Setup(c => c.GetSection("DatabaseConfig")).Returns(mockDbConfigSection.Object);
            
            _mockDbHelper.Setup(db => db.GetWrapperAsync()).ReturnsAsync(_mockMySqlConnectorWrapper.Object);
        ...
        public async Task GetMostRecentActivity_ActivityFound_ReturnsSessionList()
        {
            ...
            _mockMySqlConnectorWrapper.Setup(c => c.ExecuteReaderAsync("SessionListMostRecentActivity", CommandType.StoredProcedure, null))
                                      .ReturnsAsync(_mockDataReader.Object);
